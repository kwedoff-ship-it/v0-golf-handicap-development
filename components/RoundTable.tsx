...
export function RoundTable({ rounds, showDifferential = false, maxRows }: RoundTableProps) {
      ...
      <table className="w-full">
        <thead className="sticky top-0 bg-slate-800/90 backdrop-blur-sm z-10">
          <tr className="bg-slate-800/50">
            ...
            {showDifferential && <th className="text-left px-6 py-4 text-sm font-semibold text-slate-300">Diff</th>}
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-800">
          {displayRounds.length === 0 ? (
            <tr>
              <td colSpan={showDifferential ? 7 : 6} className="px-6 py-12 text-center text-slate-500">
            ...
            displayRounds.map((r) => {
              ...
              return (
                <tr key={r.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  {showDifferential && <td className="px-6 py-4 text-slate-300">{diff.toFixed(1)}</td>}
                </tr>
              )
            })
          )}
